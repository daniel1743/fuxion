...
// ===================================================================
// TEST 8: TELEGRAM MESSAGE V3 - Payload estructurado Fase Final 4
// ===================================================================

describe('buildTelegramMessageV3', () => {
  test('debe construir payload V3 para cliente curioso (nivel 1)', () => {
    const profile = createLeadProfile({
      sessionId: 'test-v3-1',
      messages: [makeUserMsg('Qué es Prunex?')],
      productNames: ['prunex'],
    ...
    const payload = buildTelegramMessageV3(profile);
    expect(payload).toBeDefined();
    expect(payload.version).toBe('v3');
    expect(payload.intencion.nivel).toContain('Curioso');
    expect(payload.intencion.score).toBe(20);
    expect(payload.productos.principal).toBe('prunex');
    expect(payload.productos.porCategoria).toBeDefined();
    expect(payload.productos.resumenCategorias).toContain('Bienestar digestivo');
    expect(payload.textoFormateado).toBeDefined();
  });

  test('debe construir payload V3 para cliente interesado (nivel 2)', () => {
    const profile = createLeadProfile({
      sessionId: 'test-v3-2',
      messages: [
        makeUserMsg('Qué es Prunex?'),
    ...
    const payload = buildTelegramMessageV3(profile);
    expect(payload.intencion.nivel).toContain('Interesado');
    expect(payload.intencion.score).toBe(40);
    expect(payload.señales).toContain('pregunta precio');
  });

  test('debe construir payload V3 para posible compra (nivel 3)', () => {
    const profile = createLeadProfile({
      sessionId: 'test-v3-3',
      messages: [
        makeUserMsg('Quiero comprar ON'),
      ...
      productNames: ['on'],
      score: 70,
      detectedSignals: ['quiere comprar', 'pregunta precio', 'consulta despacho'],
      lastMessage: 'Hacen envíos?',
      isBusinessIntent: false
    });

    const payload = buildTelegramMessageV3(profile);
    expect(payload.intencion.nivel).toContain('Posible compra');
    expect(payload.intencion.score).toBe(70);
    expect(payload.productos.principal).toBe('on');
    expect(payload.productos.porCategoria).toBeDefined();
    expect(payload.señales.length).toBeGreaterThanOrEqual(3);
  });

  test('debe construir payload V3 para cliente caliente (nivel 4)', () => {
    const profile = createLeadProfile({
      sessionId: 'test-v3-4',
      messages: [
        makeUserMsg('Quiero comprar ON ahora mismo'),
        makeUserMsg('Cómo pago?'),
        makeUserMsg('Mi whatsapp es 912345678')
      ],
      productNames: ['on'],
      score: 90,
      detectedSignals: ['quiere comprar'],
      lastMessage: 'Mi whatsapp es 912345678',
      isBusinessIntent: false
    });

    const payload = buildTelegramMessageV3(profile);
    expect(payload.intencion.nivel).toContain('Cliente caliente');
    expect(payload.intencion.score).toBe(90);
    expect(payload.contacto.disponible).toBe(true);
  });

  test('debe construir payload V3 para oportunidad de negocio', () => {
    const profile = createLeadProfile({
      sessionId: 'test-v3-5',
      messages: [makeUserMsg('Quiero ser distribuidor FuXion')],
      productNames: [],
    ...
    const payload = buildTelegramMessageV3(profile);
    expect(payload.intencion.nivel).toContain('OPORTUNIDAD NEGOCIO');
    expect(payload.intencion.isBusinessIntent).toBe(true);
    expect(payload.productos.principal).toBe('Sin producto específico');
  });

  test('debe agrupar productos por categoría en payload V3', () => {
    const profile = createLeadProfile({
      sessionId: 'test-v3-6',
      messages: [
        makeUserMsg('Me interesa Prunex y también ON')
      ],
      productNames: ['prunex', 'on'],
      score: 50,
      lastMessage: 'Me interesa Prunex y también ON',
    ...
    const payload = buildTelegramMessageV3(profile);
    expect(payload.productos.porCategoria).toBeDefined();
    expect(payload.productos.porCategoria['Bienestar digestivo']).toContain('prunex');
    expect(payload.productos.porCategoria['Energía y vitalidad']).toContain('on');
    expect(payload.productos.resumenCategorias).toContain('Bienestar digestivo');
    expect(payload.productos.resumenCategorias).toContain('Energía y vitalidad');
  });

  test('debe detectar repeticiones de productos en payload V3', () => {
    const profile = createLeadProfile({
      sessionId: 'test-v3-7',
      messages: [
        makeUserMsg('Qué es Prunex?'),
        makeUserMsg('Y Prunex es bueno?'),
        makeUserMsg('Dónde compro Prunex?')
      ],
      productNames: ['prunex', 'prunex', 'prunex'],
      score: 60,
      detectedSignals: ['consulta repetida del mismo producto'],
      lastMessage: 'Dónde compro Prunex?',
      isBusinessIntent: false
    });

    const payload = buildTelegramMessageV3(profile);
    expect(payload.productos.repeticiones.length).toBeGreaterThan(0);
    expect(payload.productos.repeticiones[0]).toContain('mismo producto');
  });
});

// ===================================================================
// TEST 9: TELEGRAM TEXT V3 - Texto formateado desde payload V3
// ===================================================================

describe('buildTelegramTextV3', () => {
  test('debe generar texto formateado desde payload V3', () => {
    const profile = createLeadProfile({
      sessionId: 'test-text-1',
      messages: [makeUserMsg('Quiero comprar ON')],
      productNames: ['on'],
      score: 80,
      detectedSignals: ['quiere comprar'],
    ...
    const payload = buildTelegramMessageV3(profile);
    const text = buildTelegramTextV3(payload);

    expect(text).toBeDefined();
    expect(text).toContain('Falcon Assistant - Lead Intelligence V3');
    expect(text).toContain('Posible compra');
    expect(text).toContain('Score: 80%');
    expect(text).toContain('on');
    expect(text).toContain('Quiere comprar');
  });

  test('debe incluir señales con emojis en texto V3', () => {
    const profile = createLeadProfile({
      sessionId: 'test-text-2',
      messages: [
        makeUserMsg('Cuánto cuesta Prunex?'),
        makeUserMsg('Hacen envíos?')
      ],
    ...
    expect(text).toContain('📬');
    expect(text).toContain('Preguntó precio');
    expect(text).toContain('Consultó despacho');
  });

  test('debe incluir información de contacto en texto V3', () => {
    const profile = createLeadProfile({
      sessionId: 'test-text-3',
      messages: [
        makeUserMsg('Quiero comprar ON, mi teléfono es 912345678')
      ],
      productNames: ['on'],
      score: 80,
      detectedSignals: ['quiere comprar'],
      lastMessage: 'Quiero comprar ON, mi teléfono es 912345678',
      isBusinessIntent: false
    });
    ...
    expect(text).toContain('Contacto disponible');
    expect(text).toContain('telefono');
  });

  test('debe incluir categorías de productos en texto V3', () => {
    const profile = createLeadProfile({
      sessionId: 'test-text-4',
      messages: [
        makeUserMsg('Me interesa Prunex y ON')
      ],
      productNames: ['prunex', 'on'],
      score: 50,
      lastMessage: 'Me interesa Prunex y ON',
      isBusinessIntent: false
    });
    ...
    expect(text).toContain('Bienestar digestivo');
    expect(text).toContain('Energía y vitalidad');
  });

  test('debe incluir sugerencia en texto V3', () => {
    const profile = createLeadProfile({
      sessionId: 'test-text-5',
      messages: [makeUserMsg('Quiero comprar ON')],
      productNames: ['on'],
      score: 80,
      detectedSignals: ['quiere comprar'],
