...
import { INTENT_LEVELS, PRODUCT_CATEGORY_MAP } from '../config/chatAlertRules.js';
...
// ===================================================================
// 8. TELEGRAM MESSAGE BUILDER V2 - Mensaje enriquecido CRM
// ===================================================================
...
// ===================================================================
// 9. TELEGRAM MESSAGE BUILDER V3 - Payload estructurado Fase Final 4
// ===================================================================

/**
 * Construye un payload estructurado V3 para Telegram con:
 * - Nivel de intención (4 niveles)
 * - Score y label
 * - Producto principal con categoría
 * - Productos secundarios agrupados por categoría
 * - Señales detectadas
 * - Resumen inteligente de productos
 * 
 * @param {Object} leadProfile - Perfil de lead generado por createLeadProfile()
 * @returns {Object} Payload estructurado con datos para Telegram
 */
export const buildTelegramMessageV3 = (leadProfile) => {
  const { cliente, tiempoChat, mensajes, productoPrincipal, productosSecundarios, etapa, interes, contacto, ultimaFrase, sugerencia, raw } = leadProfile;
  
  // ===================================================================
  // 1. Determinar nivel de intención (4 niveles)
  // ===================================================================
  let intentLevel;
  if (raw.isBusinessIntent) {
    intentLevel = INTENT_LEVELS.business;
  } else if (raw.score >= INTENT_LEVELS.hotClient.min) {
    intentLevel = INTENT_LEVELS.hotClient;
  } else if (raw.score >= INTENT_LEVELS.possiblePurchase.min) {
    intentLevel = INTENT_LEVELS.possiblePurchase;
  } else if (raw.score >= INTENT_LEVELS.interested.min) {
    intentLevel = INTENT_LEVELS.interested;
  } else {
    intentLevel = INTENT_LEVELS.curious;
  }
  
  // ===================================================================
  // 2. Clasificar productos por categoría
  // ===================================================================
  const allProducts = [productoPrincipal, ...productosSecundarios].filter(p => p !== 'Sin producto específico');
  const productsByCategory = {};
  
  for (const product of allProducts) {
    const productLower = product.toLowerCase().trim();
    const category = PRODUCT_CATEGORY_MAP[productLower] || 'Otros productos';
    if (!productsByCategory[category]) {
      productsByCategory[category] = [];
    }
    if (!productsByCategory[category].includes(product)) {
      productsByCategory[category].push(product);
    }
  }
  
  // ===================================================================
  // 3. Detectar repeticiones de productos
  // ===================================================================
  const productRepetitions = [];
  if (raw.detectedSignals && raw.detectedSignals.includes('consulta repetida del mismo producto')) {
    productRepetitions.push('⚠️ El cliente ha preguntado por el mismo producto en múltiples ocasiones');
  }
  
  // ===================================================================
  // 4. Construir resumen inteligente de productos
  // ===================================================================
  const categorySummary = Object.entries(productsByCategory)
  ...
  const payload = {
    ...
    tiempo: {
      ...
      preguntas: mensajes.preguntas
    },
    
    // Intención (4 niveles)
    intencion: {
      nivel: intentLevel.label,
      score: raw.score,
      etapa: etapa.id,
      etapaLabel: etapa.label,
      etapaSignificado: etapa.significado,
      isBusinessIntent: raw.isBusinessIntent
    },
    
    // Productos
    productos: {
      principal: productoPrincipal,
      secundarios: productosSecundarios,
      porCategoria: productsByCategory,
      resumenCategorias: categorySummary,
      repeticiones: productRepetitions
    },
    
    // Señales detectadas
    señales: raw.detectedSignals || [],
    
    // Contacto
    contacto: {
      disponible: contacto.disponible,
      datos: contacto.contactos,
      resumen: contacto.resumen
    },
    
    // Último mensaje
    ultimoMensaje: ultimaFrase,
    
    // Sugerencia para el asesor
    sugerencia: sugerencia,
    
    // Texto formateado para Telegram (legacy compatibility)
    textoFormateado: buildTelegramMessageV2(leadProfile)
  };
...
// ===================================================================
// 10. BUILD TELEGRAM TEXT V3 - Texto formateado desde payload V3
// ===================================================================

/**
 * Convierte un payload V3 en texto formateado para Telegram.
 * 
 * @param {Object} payloadV3 - Payload generado por buildTelegramMessageV3()
 * @returns {string} Texto formateado para Telegram
 */
export const buildTelegramTextV3 = (payloadV3) => {
  const { cliente, tiempo, intencion, productos, señales, contacto, ultimoMensaje, sugerencia } = payloadV3;
  ...
  // ===================================================================
  lines.push(`🤖 Falcon Assistant - Lead Intelligence V3`);
  lines.push('');
  lines.push(`${intencion.nivel}`);
  lines.push('');
  ...
  lines.push(`  Score: ${intencion.score}%`);
  lines.push(`  Etapa: ${intencion.etapaLabel}`);
  lines.push(`  ${intencion.etapaSignificado}`);
  lines.push('');
  
  // ===================================================================
  // Productos con categorías
  // ===================================================================
  if (productos.principal !== 'Sin producto específico') {
    lines.push(`📦 Productos:`);
    lines.push(`  Principal: ${productos.principal}`);
    if (productos.secundarios.length > 0) {
      lines.push(`  Secundarios: ${productos.secundarios.join(', ')}`);
    }
    if (productos.resumenCategorias) {
      lines.push(`  Categorías:`);
      lines.push(productos.resumenCategorias);
    }
    if (productos.repeticiones.length > 0) {
      lines.push(`  ${productos.repeticiones.join('\n  ')}`);
    }
    lines.push('');
  }
  ...
  if (señales.length > 0) {
    const signalMap = {
      'quiere comprar': '🛒 Quiere comprar',
      'pregunta precio': '💰 Preguntó precio',
      'consulta despacho': '📬 Consultó despacho',
      'problema específico': '🎯 Necesidad específica',
      'pregunta beneficios': '📋 Preguntó beneficios',
      'pregunta ingredientes': '🔬 Preguntó ingredientes',
      'compara productos': '⚖️ Comparó productos',
      'consulta repetida del mismo producto': '🔄 Insistió en mismo producto'
    };
    const humanSignals = señales
      .map(s => signalMap[s] || `🔍 ${s}`)
      .filter(Boolean);
    lines.push(`🔎 Señales:`);
    humanSignals.forEach(s => lines.push(`  ${s}`));
    lines.push('');
  }
  ...
  // ===================================================================
  lines.push(`⏱ Actividad:`);
  lines.push(`  ${tiempo.minutos} min · ${tiempo.mensajes} mensajes · ${tiempo.preguntas} preguntas`);
  lines.push('');
  ...
  if (ultimoMensaje) {
    ...
    lines.push(`  "${ultimoMensaje.length > 120 ? ultimoMensaje.substring(0, 120) + '...' : ultimoMensaje}"`);
    lines.push('');
  }
  ...
  lines.push(`📞 Contacto:`);
  lines.push(`  ${contacto.resumen}`);
  if (contacto.disponible && contacto.datos.length > 0) {
    contacto.datos.forEach(d => {
      lines.push(`  ${d.tipo}: ${d.valor}`);
    });
  }
  lines.push('');
  ...
  // ===================================================================
  // Business intent extra
  // ===================================================================
  if (intencion.isBusinessIntent) {
    lines.push('');
    lines.push(`📞 Recomendación: Contactar personalmente para explicar oportunidad de negocio.`);
  }
