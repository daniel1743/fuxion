...
export const processChatConversation = async ({ conversation, startedAt, sessionId, response, cacheHit = false, provider = null, latencyMs = 0 }) => {
  ...
  if (cooldownResult.permitido) {
    ...
    let sent = await sendTelegramNotification({ text: message });

    if (sent) {
      // Registrar notificación enviada
      registerNotification(evaluation.sessionId, cooldownState);
      evaluation.telegramSent = true;
      evaluation.telegramError = null;
      debugLog('telegram_v2_sent', true);
    } else {
      console.warn('[chatEvents] telegram_v2_failed, intentando fallback V1');
      // Fallback: si V2 falla, enviar con formato antiguo V1 como respaldo
      ...
      sent = await sendTelegramNotification({ text: fallbackMessage });

      if (sent) {
        registerNotification(evaluation.sessionId, cooldownState);
        evaluation.telegramSent = true;
        evaluation.telegramError = null;
        debugLog('telegram_fallback_sent', true);
      } else {
        evaluation.telegramSent = false;
        evaluation.telegramError = 'telegram_failed';
    ...
    // Cooldown activo - no enviar nada
    debugLog('telegram_cooldown_active', cooldownResult.razon);
    evaluation.telegramSent = false;
